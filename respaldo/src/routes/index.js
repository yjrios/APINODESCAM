...
router.get('/vehiculos',(req,res)=>{
    const sql = 'SELECT @numero:=@numero + 1 AS posicion, v.* , m.*, st.status FROM maestro_vehiculo v INNER JOIN marca m INNER JOIN status st ON v.id_marca = m.id AND v.id_status=st.id, (SELECT @numero:= 0) AS s;';
...
router.get('/vehiculos/:id',(req,res)=>{
    ...
    const sql = `SELECT *
     FROM maestro_vehiculo v INNER JOIN marca m INNER JOIN status s INNER JOIN tipos_de_carga t INNER JOIN kilometraje k
     ON v.id_marca = m.id AND v.id_tipo_carga=t.id AND v.id_status = s.id AND k.id_vehiculo=v.id
     WHERE placa = '${id}'
...
router.get('/kilometraje/:id',(req,res)=>{
    const {id} = req.params
    const sql = `SELECT k.kilometraje AS km, k.fecha, k.id as id_km, v.* ,v.id as id_v , m.* , s.*,t.*, o.fecha as fstatus, o.id as id_ultimo_s
...
router.post('/actualizarst', (req,res)=>{
    ...
    function insertars (){
        ...
        function actualizar (id_nuevo_status){
            ...
            SET o.dias = '${dias}' , o.id_sucesor = '${id_nuevo_status}',   m.id_status = '${id}'
...
router.get('/getHistorico/',(req,res)=>{
    ...
    const placa = req.query.placa
     const sql = `SELECT * FROM operatividad o 
     INNER JOIN status s
...
router.get('/getMtto/:nro',(req,res)=>{
    ...
    const sql = `SELECT m . * , d . * , p . * , s . * , t . * , k.kilometraje , v.*, sp.status_pago, sp.id as id_p, sm.status_mtto
     FROM mantenimiento m 
     INNER JOIN detalle_mttos d 
     INNER JOIN maestro_servicio s
     INNER JOIN maestro_proveedores p
...
router.put('/actualizarmtto', (req,res)=>{
    ...
    // console.log("actulizar : " + actualizar)
    // console.log("status : " + statusm)
    // console.log("id kilometro : " + req.body.id_km)
