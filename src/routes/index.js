...
router.put('/update/:id', (req, res)=>{
   ...
   connection.query (sql, error=> { res.send('Customer updated!');
    });

});

router.post ('/upload/:placa', upload.single('file'), (req, res) => {
   
    const id = req.params.placa
    const dirfoto = req.file.filename
    //console.log('nombre: '+ req.file.filename)
    //console.log('placa: '+ id)
    const sql = `UPDATE maestro_vehiculo SET ? WHERE placa ='${id}' `;
...
router.put('/updatecar/:placa', (req, res) => {
...
router.get('/getStatusPeriodo/',(req,res)=>{
    const hasta = req.query.hasta
     ...
     WHERE o.fecha BETWEEN '0000-00-00' AND '${hasta}'
     GROUP BY o.placa`
    connection.query(sql,(error, result)=>{
        if(error) {
            const response = {message: 'Error!', data: {error} }
...
const controladormodificar = require('./controllers/ModificarViajes')
const controladorstatus = require('./controllers/ObtenerStatus')
const controladoranular = require('./controllers/AnularViaje')
const controladorfindviaje = require('./controllers/FindViaje')
const controladorlogin = require('./controllers/LoginAmc')
const controladoreventos = require('./controllers/FindEventos')
